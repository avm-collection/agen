...
func (a *AGEN) GetInstAt(addr Word) *Inst {
	return &a.program[addr]
}

func (a *AGEN) AddInstWith(str string, data Word) Word {
	addr := Word(len(a.program));
	a.program = append(a.program, Inst{Op: Insts[str].Op, Data: data})
	return addr
}

func (a *AGEN) AddInst(str string) Word {
